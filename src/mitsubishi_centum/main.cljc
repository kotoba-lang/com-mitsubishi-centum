(ns mitsubishi_centum.main
  "Kotodama WASM entrypoint for the Mitsubishi_centum clean-room actor (L4) — Clojure port.
  ...
  facts (`mitsubishi_centum.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mitsubishi_centum")
(def tier "L4")
...
(def entity-specs
  [{:entity "Tag"        :plural "tags"        :id-prefix "mitsubis_tag"
   ...
   {:entity "ControlLoop" :plural "controlloops" :id-prefix "mitsubis_con"
    :fields [:name :setpoint :processValue :mode] :required [:name :setpoint]
    :coerce {:setpoint :float :processValue :float} :refs {}}
   {:entity "Batch"      :plural "batches"     :id-prefix "mitsubis_bat"
...
(defn emit-facts
  "EAVT facts for one record: {\"mitsubishi_centum.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mitsubishi_centum-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mitsubishi_centum-compat/ready)
