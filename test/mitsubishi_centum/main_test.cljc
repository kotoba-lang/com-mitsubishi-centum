(ns mitsubishi_centum.main-test
  "Contract + behavioral test for the mitsubishi_centum-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "mitsubishi_centum-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'mitsubishi_centum.main-test)]
